...
window.addEventListener("load", async () => {
    // Update progress while loading real data
    updateLoadingProgress(30, 'Connecting to database...');

    // ensure unique viewer row exists
    await registerViewer();
    updateLoadingProgress(50, 'Loading statistics...');

    // initial stats
    await updateStats();
    updateLoadingProgress(70, 'Loading artworks...');

    // subscribe for realtime updates of stats
    subscribeStatsRealtime();
    updateLoadingProgress(85, 'Setting up gallery...');

    // then keep your existing initial data + render
